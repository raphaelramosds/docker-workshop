...
- PORTS: portas expostas pelo container (não aplicável nesse exemplo)
- NAMES: nome do container (definido via flag `--name` no comando `docker run`)
...
Escrevemos nossa própria versão da imagem `hello-world`. Mas, ao invés de seu entrypoint ser um script que apenas imprime parágrafos e termina sua execução, fizemos com que ela execute um script Python que imprima "Hello World" a cada 5 segundos indefinidamente.

Abaixo o passo a passo para construir e executar essa imagem
